<template>
  ...
  >
        ...
        >
          <div class="justify-between flex w-full">
            ...
            >
              <NuxtLink
                @click="newChat()"
                class="btn-secondary dark:bg-dark-primary dark:text-gray-400 cursor-pointer w-96"
              >
                <svg
                  width="20"
                  height="20"
              ...
              <button
                class="btn-secondary dark:bg-dark-primary"
                @click="toggleSidebar()"
              >
                <span>
                  <svg
              ...
              <div>
                <span>
                  <div class="relative h-[90vh]" style="height: auto">
                    <ol>
                      <li
                        v-if="loadList == 'loaded'"
                        class="relative z-[15] h-auto my-2"
                      ...
                      >
                        <div
                          class="sticky top-2 z-40 dark:text-background-hover-secondary"
                          v-if="
                            questionTemplates[index].history_category !==
                            questionTemplates[index - 1]?.history_category
                          "
                        >
                          ...
                          {{
                            $t(
                              `${
                                templatechat.history_category
                                  .charAt(0)
                                  .toUpperCase() +
                                templatechat.history_category.slice(1)
                              }`,
                            )
                          }}
                        ...
                        >
                          ...
                          >
                            <p v-if="templatechat.isRename ? false : true">
                              {{
                                templatechat.history_title.length > 10
                                  ? templatechat.history_title.substring(
                                      0,
                                      12,
                                    ) + '...'
                          ...
                          <div
                            class="flex gap-2"
                            :class="
                              questionTemplates[index].isEdit ? '' : 'hidden'
                            "
                          >
                            <img
                              v-if="templatechat.isRename ? false : true"
                              @click="renameHistory(index)"
                              @click.stop
                            ...
                            <img
                              v-if="templatechat.isRename ? false : true"
                              @click="deletHistory(templatechat.history_id)"
                              @click.stop
                            ...
                            <div
                              v-else
                              @click.stop
                              class="cursor-pointer border border-black p-1 rounded-lg"
                              @click="
                                historyRename(
                                  templatechat.history_id,
                                  titleTemp,
                                )
                              "
                            >
                      ...
                      <li
                        v-else-if="loadList == 'loading'"
                        class="relative z-20 h-auto my-2"
                      >
          ...
          <footer class="border-t border-gray-50 dark:border-0 pt-2">
            <div
              ...
              :class="isOpen ? 'items-center' : 'items-start'"
            >
              <NuxtLink
                to="/auth/change-password/direct"
                class="justify-start items-center gap-3 inline-flex dark:text-gray-400 mb-3"
              >
              ...
              <NuxtLink
                @click="logOut"
                to="#"
                class="justify-start items-center gap-3 inline-flex dark:text-gray-400 mb-20"
              >
    ...
    <ModalsConfirm
      v-show="show_modal_logout"
      :title="$t('logout__modal_title')"
      :description="$t('logout__modal_description')"
      :button_yes="$t('logout__modal_button_yes')"
      :button_no="$t('logout__modal_button_no')"
      @send-to="sureLogout"
      @close-confirm="show_modal_logout = !show_modal_logout"
    />

    <ModalsError
      v-show="show_modal_error"
      title="Error"
      description="Failed to Logout"
      @close-confirm="show_modal_error = !show_modal_error"
    />
  </div>
</template>
...
const show_modal_logout = ref(false);
const show_modal_error = ref(false);
...
const historyRename = async (history_id: any, history_title: any) => {
  ...
  chatStore.renameChat(history_id, history_title).then(() => {
...
const deletHistory = async (val: any) => {
  ...
  if (confDel) {
    chatStore.deleteChatById(val).then((val: any) => {
...
const sureLogout = () => {
  if (process.client) {
    authStore
      ...
      .catch((err: any) => {
        show_modal_error.value = !show_modal_error.value;
        show_modal_logout.value = !show_modal_logout.value;
        throw err;
      });
  }
};
