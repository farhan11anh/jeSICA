<template class="h-screen">
  <div :class="theme == 'dark' ? 'dark' : ''" class="font-inter relative">
    ...
    <transition name="fade" mode="out-in">
      ...
      <div
        v-if="animation"
        class="fixed left-5 z-30 w-40 h-20"
        :class="appear_settings ? 'bottom-5' : '-bottom-16'"
      >
        <div
          class=" p-5 w-full h-full box-border rounded-2xl"
        >
          <!-- lang changer -->
          <div class="locale-changer absolute bottom-5 left-5">
            ...
            <transition name="lang">
              <div
                ...
                class="w-12 cursor-pointer border border-black rounded-full"
              >
                <img class="w-full" src="/img/en-flag.svg" alt="" />
              </div>
              <div v-else @click="$i18n.locale = 'en'" 
              class="w-12 cursor-pointer border border-black rounded-full">
