          ...
          <div class="py-2 px-4 mx-auto max-w-screen-xl text-center lg:py-3">
            <div class="content py-16 px-24 text-left">
              ...
              <div class="flex gap-5 justify-between flex-wrap mt-5">
                <div class="grow">
                  <div
                    class="w-full h-10 mx-auto lg:h-[75px] lg:w-[75px] xl:w-28 xl:h-28 border border-gray-300 relative box-border rounded-lg input-group buble-jsc"
                  >
                    <input
                      class="otp absolute w-full top-1/2 -translate-y-1/2 text-center text-4xl h-full rounded-lg"
                      type="text"
                      maxlength="1"
                      v-model="input1"
                      @input="moveToNextInput($event, 2)"
                    />
                  </div>
                </div>
                <div class="grow">
...
export default {
  ...
  methods: {
    ...
    async send() {
      ...
      try {
        await this.authStore.setToken(otp, email).then((resp) => {
          // console.log(resp);
          const token = resp.data.data.access_token;
          try {
            // saveDataFromToken(token)
            // save user data
            this.authStore.verificationToken(token).then((UserData) => {
